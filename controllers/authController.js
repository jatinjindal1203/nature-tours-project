...
const createSendToken = (user, statusCode, res) => {
    ...
    // res.cookie(name-of-the-cookie, data-want-to-send-in-cookie, optons: { expires: amount-of-time-after-which-cookie-expires, secure: cookie-only-be-send-on-https, httpOnly: Browser-will-only-read-and-send-back-it-with-every-request-and-can't-modify-it, and_many_more.... })
    const cookieOptions = {
...
exports.restrictTo = (...roles) => (req, res, next) => {
        ...
        if(!roles.includes(req.user.role)){
            return next(new AppError('You do not have premission to perform this sction', 403));
        }
        next();
    };
...
exports.resetPassword = catchAsync( async (req, res, next) => {
    ...
    // 3) Update changePasswordAt prperty for the user
